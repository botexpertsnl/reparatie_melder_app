...
export function RepairDetailsPanel({
  ...
  return (
    <aside
      className={className ?? "relative border-l border-[#253149] bg-[#0b1221] px-5 py-5"}
      style={style}
    >
      ...
      <div className="mt-4 border-t border-[#253149] pt-4 text-sm text-slate-300">
        {repair.description}
      </div>
      <div className="mt-5 space-y-2">
        {["Diagnosing", "Repairing", "Ready for Pickup"].map((step) => (
          <button
            key={step}
            type="button"
            className="flex w-full items-center justify-between rounded-xl border border-[#253149] px-3 py-2 text-left text-sm text-slate-200 hover:bg-[#182236]"
          >
            {step}
      ...
      {onLinkChange ? (
        <button
          ...
          className="absolute bottom-5 right-5 text-[#69f0df] hover:text-[#25d3c4]"
